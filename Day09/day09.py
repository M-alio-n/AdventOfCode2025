...
def legal_field(t1: tuple, t2: tuple, color_fields: set):
    # Because we know that there are no pockets, its enough to check the edges of the rectangles
    for dim_0 in range(min([t1[0], t2[0]]), max([t1[0], t2[0]])+1):
        if not (dim_0, t1[1]) in color_fields or not (dim_0, t2[1]) in color_fields:
            return False
    for dim_1 in range(min([t1[1], t2[1]]), max([t1[1], t2[1]])+1):
        if not (t1[0], dim_1) in color_fields or not (t2[0], dim_1) in color_fields:
            return False
    return True
